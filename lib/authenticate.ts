...
export async function saveAuthenticateSession(
  tokens: AuthenticateTokensType,
): Promise<void> {
...
export function parseAuthenticateSessionCookie(cookie: string): {
  refresh: string;
  access: string;
  access_exp: number;
} {
...
export async function isAuthenticateSessionAccessTokenExpired(
  exp?: number,
): Promise<boolean> {
  if (!exp) {
