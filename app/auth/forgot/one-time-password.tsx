...
import { AuthenticationCheck, AuthenticationOneTimePassword, AuthenticationPassword, ForgotPasswordOneTimePassword, LoginUser } from "@/lib/actions/authenticate"
...
export function ForgotOneTimePasswordSection({ username, setSection }: PropsType) {
  ...
  async function onSubmit(values: z.infer<typeof schemaAuthenticateOneTimePassword>) {
    ...
    setLoading(true)
    const result = await ForgotPasswordOneTimePassword(username, values.code)
    ...
    if (result.success) {
      ...
      setSection(ForgotPasswordSectionEnum.RESET)
    } else {
      form.setValue('code', '')
