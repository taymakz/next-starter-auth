...
const generatePoints = (length: number) =>
  Array.from({ length }).fill(0).map(() => [Math.random(), Math.random()]);
...
export function AnimatedBackground() {
  ...
  return (
    ...
    >
      <div
        className="aspect-[1.7] h-full w-full bg-gradient-to-r from-primary to-white/10 xs:opacity-50 lg:opacity-30"
