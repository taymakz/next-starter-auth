...

  {
    variants: {
      variant: {
        ...
        "floating-label":
          "relative block  [&_input]:px-4 [&_input]:py-3.5 [&_input]:rounded-lg focus-within:border-primary cursor-text [&_input]:border-none [&_input]:bg-transparent [&_input]:w-full [&_input]:placeholder-transparent [&_input]:focus:border-transparent [&_input]:focus:outline-none [&_input]:outline-none [&_input]:placeholder:text-right",
      },
    },
    defaultVariants: {
      variant: "default",
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const [togglePasswordReveal, setTogglePasswordReveal] =
          ...
          <div className="relative w-full">
            ...
            >
              ...
              {label && (
                <span
                  className={cn(
                    "pointer-events-none absolute start-2.5 top-0 -translate-y-1/2 rounded-lg bg-secondary px-2 py-0.5 text-xs text-muted-foreground transition-all peer-placeholder-shown:top-1/2 peer-placeholder-shown:text-sm peer-focus:top-0 peer-focus:text-xs peer-focus:text-primary",
                    labelClass,
                  )}
                >
                  {label}
...
function PasswordReveal({
  ...
  return (
    <span className="absolute inset-y-0 end-4 flex items-center justify-center">
      <span
        className="cursor-pointer text-muted-foreground"
        onClick={() => setTogglePasswordReveal(!togglePasswordReveal)}
      >
